...
//does it start at a NULL location
	if(head ==NULL)
		return 0;
	...
	while(1){
		...
		if(ptr2 == NULL) //prevent attempts to reach space the array that doesnt exist
			return 0;

		if(ptr1 == ptr2 || ptr1 == (ptr2->next)) //cycle if pointers have same value or  first pointer is equal to second pointer incremented
			return 1;
